# ph-core/oamd-bio-workflow-neissflow: Usage

> _Documentation of pipeline parameters is generated automatically from the pipeline schema and can no longer be found in markdown files._

## Introduction

neissflow can be ran from the command line or AMD Platform
...
To generate a samplesheet of paired-end FASTQ files use the script found at `bin/fastq_dir_to_samplesheet.py`. The directory used to generate the samplesheet should ONLY contain samples to be analyzed as part of the run.
...
```
$ python3 bin/fastq_dir_to_samplesheet.py <FASTQ DIR> <SAMPLESHEET_FILE>
```
...
```bash
nextflow run ph-core/oamd-bio-workflow-neissflow -profile docker -params-file params.yaml
```
...
```bash
nextflow pull ph-core/oamd-bio-workflow-neissflow
```
...
First, go to the [ph-core/oamd-bio-workflow-neissflow releases page](https://github.com/ph-core/oamd-bio-workflow-neissflow/releases) and find the latest pipeline version - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`. Of course, you can switch to another version by changing the number after the `-r` flag.
