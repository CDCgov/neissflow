...
```bash
nextflow run CDCgov/neissflow -profile docker -params-file params.yaml
```
...
```bash
nextflow pull CDCgov/neissflow
```
...
First, go to the [CDCgov/neissflow releases page](https://github.com/CDCgov/neissflow/releases) and find the latest pipeline version - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`. Of course, you can switch to another version by changing the number after the `-r` flag.
